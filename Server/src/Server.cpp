...
{
	std::u16string filePath = u"Monitors.kds";
	BinaryReader reader(filePath);
	if (!reader.isOpen())
		return;

	std::u16string header = reader.readU16String();
	if (header != u"Kolobenina")
	{
		reader.close();
		return;
	}
...
void Server::saveMonitors()
{
	...
	writer.writeU16String(u"Kolobenina");
...
{
	std::u16string filePath = u"Scanners.kds";
	BinaryReader reader(filePath);
	if (!reader.isOpen())
		return;
...
void Server::startUp()
{
	base = std::shared_ptr<Base>(BaseLoader::load(u"Base.kds"));
	threats = std::make_shared<ThreatList>(u"Threats.kds");
	threats->load();
